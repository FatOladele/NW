...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    playtile (state) {
      ...
      for (var i of Object.values(state.tonboard)) {
        if (i > 200) {
          i = 200
        }
  ...
  actions: {
    ...
    vscomp (context, data) {
      const settings = { time: data.time, rules: data.rules, creator: data.creator }
      context.commit('sethost', true)
      context.commit('mutgsetting', settings)
      context.commit('updatelanguage', data.language)
    },
    SOCKET_gid (context, data) {
    ...
    SOCKET_update_room (context, data) {
      if (Object.keys(this.state.settings).length === 0) {
        ...
        context.commit('mutgsetting', settings)
        context.commit('updatelanguage', data.roomdata.language)
    ...
    SOCKET_post_play (context, data) {
      ...
      if (stat === 't') {
        if (player !== this.state.gamernick) {
          ...
          const path = this._vm.$myhost + '/api/refillrack'
          axios.post(path, payload).then((res) => {
            console.log('sendng update rack')
            if (res.data.status === 'failed') {
              alert(res.data.msg)
            } else {
              const rack = res.data.rack
              ...
              this._vm.$socket.emit('updated_rack', { room: this.state.gameId })
              if (data.turnnick === 'Computer') {
                console.log(this.state.settings.rules)
                this._vm.$socket.emit('get_comp', { room: this.state.gameId, rule: this.state.settings.rules })
              }
            }
          }).catch((error) => {
            console.log(error)
